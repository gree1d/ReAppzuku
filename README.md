...
# Что такое ReAppzuku?
ReAppzuku - это форк оригинального Appzuku(Shappky) с новыми возможностями и полностью бесплатным фунционалом.

Инструмент для контроля фоновых приложений, используя Shizuku или Root-права доступа. Поддерживает ручное отключение одним нажатием, очистку по расписанию и постоянное ограничение фонового режима для выбранных приложений. Позволяет контролировать приложения, которые тратят впустую оперативную память, слишком часто перезапускаются или продолжают работать в фоновом режиме, активно потребляя заряд батареи.

## Особенности
- **Гибкие разрешения**: Работает с Shizuku или Root-правами.
...
- **Повторное применение фоновых ограничений**: Повторно примените сохраненные фоновые ограничения и просмотрите последние результаты/ошибки в журналах.
...
- **Темы и акценты**: Поддержка светлой, темной и AMOLED тем. Покраска приложения с помощью предустановленных цветовых акцентов.
- **Защищенные приложения**: критически важные для системы приложения автоматически защищены от Auto-Kill и фоновых ограничений, предостерегая вас от bootloop.
...
- **Android 11+ для работы фоновых ограничений**: Приложение устанавливается на Android 6+, но для функции "Фоновые ограничения" требуется Android 11 и выше.

## Как это работает
- **Главный экран**: Отключите выбранные запущенные приложения вручную или добавьте их в один из нескольких списков.
...
- **Фоновые ограничения**: На Android 11+ режим ожидания выбранных приложений становится более агрессивным, вплоть до полной блокировки работы в фоне и автозапуска.
