...
public class ShellManager {
    ...
    public void checkShellPermissions() {
        // If root access is available, don't request Shizuku
    ...
    /**
     * Freeze a package using Shizuku Binder API (pm disable-user --user 0).
     * This is more effective than force-stop as the app cannot restart until unfrozen.
    ...
    public boolean freezePackage(String packageName) {
        ...
        if (hasShizukuPermission()) {
            try {
                ...
                Log.d(TAG, "Frozen via Binder: " + packageName);
                return true;
            } catch (Exception e) {
                Log.w(TAG, "Binder freeze failed for " + packageName + ", falling back to shell", e);
            }
        }

        // Fallback to shell (root only)
        if (hasRootAccess()) {
            return runShellCommandBlocking("pm disable-user --user 0 " + packageName);
        }

        Log.w(TAG, "freezePackage: no permission available for " + packageName);
        return false;
    }

    /**
     * Unfreeze a package using Shizuku Binder API (pm enable --user 0).
     * Should only be called from a background thread.
     *
    ...
    public boolean unfreezePackage(String packageName) {
        ...
        if (hasShizukuPermission()) {
            try {
                ...
                Log.d(TAG, "Unfrozen via Binder: " + packageName);
                return true;
            } catch (Exception e) {
                Log.w(TAG, "Binder unfreeze failed for " + packageName + ", falling back to shell", e);
            }
        }

        // Fallback to shell (root only)
        if (hasRootAccess()) {
            return runShellCommandBlocking("pm enable --user 0 " + packageName);
        }

        Log.w(TAG, "unfreezePackage: no permission available for " + packageName);
        return false;
    }
