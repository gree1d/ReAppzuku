...
public class ShappkyService extends Service {
    ...
    // Runnable posted with a 1-hour delay after screen off
    private Runnable idleFreezeRunnable;
    // True if idleFreezeRunnable has already fired (apps were frozen)
    private boolean idleFreezeFired = false;
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        switch (action) {
            ...
            case "SCREEN_OFF":
                // Start idle freeze timer — fires after 1 hour of no screen activity
                // Note: idleFreezeFired is intentionally NOT reset here — if apps were
                // already frozen, the flag must stay true so SCREEN_ON can unfreeze them
                cancelIdleFreezeTimer();
                idleFreezeRunnable = () -> {
                    Log.d(TAG, "Idle threshold reached, freezing background restricted apps");
                    idleFreezeFired = true;
                    idleFreezeRunnable = null;
                    appManager.freezeBackgroundRestrictedApps(null);
                };
                handler.postDelayed(idleFreezeRunnable, IDLE_FREEZE_DELAY_MS);
                Log.d(TAG, "Idle freeze timer started (" + (IDLE_FREEZE_DELAY_MS / 60000) + " min)");
                break;

            case "SCREEN_ON":
                // Delay check to allow keyguard to fully dismiss on HyperOS/MIUI
                handler.postDelayed(() -> {
                    android.app.KeyguardManager km = (android.app.KeyguardManager)
                            getSystemService(Context.KEYGUARD_SERVICE);
                    boolean isLocked = km != null && km.isKeyguardLocked();
                    if (!isLocked) {
                        // Cancel pending freeze timer and unfreeze any frozen apps
                        cancelIdleFreezeTimer();
                        if (idleFreezeFired) {
                            Log.d(TAG, "Screen on after idle freeze, unfreezing apps");
                            idleFreezeFired = false;
                            appManager.unfreezeBackgroundRestrictedApps(null);
                        } else {
                            Log.d(TAG, "Screen on before idle threshold, freeze timer cancelled");
                        }
                    } else {
    ...
    /**
     * Cancels the pending idle freeze timer if it exists.
     */
    private void cancelIdleFreezeTimer() {
        if (idleFreezeRunnable != null) {
            handler.removeCallbacks(idleFreezeRunnable);
            idleFreezeRunnable = null;
        }
    }
