...
public class BackgroundAppManager {
    ...
    /**
     * Freeze all apps that are in the Background Restriction list.
    ...
    public void freezeBackgroundRestrictedApps(Runnable onComplete) {
        Set<String> packages = getBackgroundRestrictedApps();
        if (packages.isEmpty()) {
            if (onComplete != null) handler.post(onComplete);
            return;
        }
    ...
    /**
     * Unfreeze all apps that are in the Background Restriction list.
